rawInput = '''Game 1: 4 green, 7 blue; 2 blue, 4 red; 5 blue, 2 green, 2 red; 1 green, 3 red, 9 blue; 3 green, 9 blue; 7 green, 2 blue, 2 red
...
def checkGame(game):
    for set in games[game]:
        for color in maxColors:
            if color in set and set[color] > maxColors[color]:
                return
    validGames.append(game)

def getMinColors(game):
    minColors = {'red': 0, 'green': 0, 'blue': 0}
    for set in games[game]:
        for color in set:
            minColors[color] = max(minColors[color], set[color])
    return minColors

totalPower = 0
for game in games:
    checkGame(game)
    minColors = getMinColors(game)

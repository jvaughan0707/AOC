...
inputSections = rawInput.split('\n\n')

class Machine:
    def __init__(self, input):
        splitInput = input.split('\n')
        aIn = splitInput[0].split('+')
        bIn = splitInput[1].split('+')
        cIn = splitInput[2].split('=')

        a = [Fraction(aIn[1].split(',')[0]), Fraction(aIn[2])]
        b = [Fraction(bIn[1].split(',')[0]), Fraction(bIn[2])]
        c = [Fraction(cIn[1].split(',')[0]), Fraction(cIn[2])]

        c[0] += 10000000000000
        c[1] += 10000000000000
        self.A = [[a[0], b[0]], [a[1], b[1]]]

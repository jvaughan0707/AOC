...
7JFF.FF-L-LJJJ-7JL7LLL|7JJ.LJJLLLFJLFJ7JJL.LL--JJLJ..JLL-FF---J.J.F.L.J.|-|LFF--7-L|-LF.J-JJJLLF--L-|J.L|L|.J-J.LLJJ-L|.|-L-F---J.LJJJ.FJL|7'''[1:]
...
for i in range(height):
    for j in range(width):
        if rows[i][j] in pipeMap:
            nodes[(i,j)] = [(i + i2, j + j2) for (i2, j2) in pipeMap[rows[i][j]]]
        elif rows[i][j] == 'S':
            start = (i,j)
nodes[start] = []
...
for pipe in pipeMap:
    dirs = pipeMap[pipe]
    if add(start, dirs[0]) in nodes and add(dirs[1], start) in nodes:
        rows[start[0]] = rows[start[0]].replace('S', pipe)

current = nodes[start][0]
path = [start, current]
...
pathPoints = {p: True for p in path}
outerRegion = {}

for i in range(len(path) - 1):
    diff = scale(sub(path[i+1], path[i]), 0.5)
    pathPoints[add(path[i], diff)] = True

up = scale(up, 0.5)
down = scale(down, 0.5)
left = scale(left, 0.5)
right = scale(right, 0.5)

def expand(point):
    if point[0] < -0.5 or point[1] < -0.5 or point[0] > height + 0.5 or point[1] > width + 0.5:
        return
    if point in outerRegion or point in pathPoints:
        return
    outerRegion[point] = True
...
for i in range(height):
    rowString = ''
    for j in range(width):
        if (i,j) in pathPoints:
            rowString += get((i,j))
        elif (i,j) in outerRegion:
            rowString += ' '
        else:
            innerCount += 1
            rowString += 'X'

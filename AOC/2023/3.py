...
..890....252......................544........809..425..............925......................................................................'''[1:]

symbol = '[^\.\d\\n]'
width = rawInput.index('\n')
...
for row in rawInput.split('\n'):
    i = 0
    while i < len(row):
        numSearch = re.search(r'\d+', row[i:])
        if numSearch:
            if rowCount not in nums:
                nums[rowCount] = []
            nums[rowCount].append((int(numSearch.group()), i + numSearch.span()[0], i + numSearch.span()[1] - 1))
            i = i + numSearch.span()[1]
...
for row in rawInput.split('\n'):
    i = 0
    while i < len(row):
        symSearch = re.search(r'[^\.\d\\n]', row[i:])

        if symSearch:
            if not rowCount in symbols:
                symbols[rowCount] = []
            symbols[rowCount].append((symSearch.group(), i + symSearch.span()[0]))
            i = i + symSearch.span()[1]
        else:
            break
    rowCount += 1
...
def checkNum(num, row, start, end):
    for d in [-1, 0, 1]:
        if row + d in symbols:
            for (s, i) in symbols[row + d]:
                if start - 1 <= i <= end + 1:
                    parts.append(num)
                    return

for row in nums:
    for (num, start, end) in nums[row]:
        checkNum(num, row, start, end)
...
def getGearRatio(row, col):
    ...
    for d in [-1, 0, 1]:
        if row + d in nums:
            for (num, start, end) in nums[row + d]:
...
total = 0
for row in symbols:
    for (s, i) in symbols[row]:
        if s != '*':
            continue
        total += getGearRatio(row, i)
